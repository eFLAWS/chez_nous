...
| **Plan 2D/3D** | `HouseholdSpatialView.jsx` — plus aucun importeur actif (`api.js`/`householdLayoutApi.js` orphelins, confirmé) | `floorPlanService.js` (blob JSONB `floor_plans.layout_data`, verrou optimiste). Monté sur `/spatial`. **Testé en conditions réelles par Paul — fonctionne**. **Éditeur refondu le 03/08/2026** (`PlanEditorView.jsx`, remplace `LayoutEditor.jsx` — grille 20px, "Ajuster" remplace le cadenas, undo/redo, séparation porte/fenêtre/passage réelle) — pas encore testé en navigateur réel |
...
frontend/
  src/
    ...
    features/
      ...
      layout-editor/    # MODIFIÉ (03/08/2026) — PlanEditorView.jsx/.css (remplace LayoutEditor.jsx/.css, refonte complète : grille 20px, "Ajuster" remplace le cadenas, undo/redo, séparation porte/fenêtre/passage réelle), RoomInspector.jsx, RoomCreateModal.jsx (inchangés), layoutGeneration.js/validateLayout.js (étendus pour porte/fenêtre/passage), roomCollision.js, layoutStorage.js, roomTypes.js, components/WallEdges.jsx (réécrit, rendu réel des 3 types)
    ...
    components/
      ui/
        Icons.jsx / AmbientGlow.jsx       # les SEULS fichiers de ce dossier réellement actifs (vérifié par chaîne d'import complète depuis AppRouter.jsx/main.jsx) — Icons.jsx +13 icônes le 03/08/2026 (refonte PlanEditorView.jsx), 64 au total
