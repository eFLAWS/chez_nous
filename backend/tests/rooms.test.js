...
test("createRoom : cas nominal, position (0,0) pour la première pièce du foyer", async () => {
  ...
  const res = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, color: "#F0DEC5", householdId , userId });
  assert.equal(res.success, true);
  assert.equal(res.data.x, 0);
  assert.equal(res.data.y, 0);
  assert.equal(res.data.width, 5);
  assert.equal(res.data.height, 4);
});
...
test("createRoom : largeur/longueur hors bornes rejetées", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const tooSmall = await svc.createRoom({ name: "Placard", type: "piece", width: 0.1, height: 0.1, householdId , userId });
  assert.equal(tooSmall.success, false);
  assert.match(tooSmall.error, /width/);

  const tooBig = await svc.createRoom({ name: "Immense", type: "piece", width: 500, height: 4, householdId , userId });
...
test("createRoom : couleur mal formée rejetée", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const res = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, color: "bleu", householdId , userId });
...
test("createRoom : householdId inexistant rejeté", async () => {
  resetData();
  const res = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId: "fantome" });
  assert.equal(res.success, false);
...
test("createRoom : positionnement automatique — 2e pièce placée à droite de la 1ère", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId , userId });
  const res = await svc.createRoom({ name: "Cuisine", type: "piece", width: 3, height: 3, householdId , userId });
  assert.equal(res.success, true);
...
test("createRoom : positionnement automatique — retour à la ligne quand la largeur de la grille est dépassée", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  await svc.createRoom({ name: "A", type: "piece", width: 6, height: 3, householdId , userId });
  await svc.createRoom({ name: "B", type: "piece", width: 5, height: 4, householdId , userId }); // total 11, tient encore sur la ligne (< 12)
  const res = await svc.createRoom({ name: "C", type: "piece", width: 4, height: 2, householdId , userId }); // 11 + 4 > 12 -> nouvelle ligne
  assert.equal(res.success, true);
  assert.equal(res.data.x, 0);
...
test("createRoom : le placement automatique ne modifie jamais les pièces déjà existantes", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const first = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId , userId });
  await svc.createRoom({ name: "Cuisine", type: "piece", width: 3, height: 3, householdId , userId });
...
test("createRoom : position explicite (x, y) respectée si fournie", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const res = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId, x: 2, y: 7 , userId });
  assert.equal(res.success, true);
...
test("listRooms : filtre bien par foyer, deux foyers ne se mélangent jamais", async () => {
  ...
  await svc.createRoom({ name: "Salon H1", type: "piece", width: 5, height: 4, householdId: h1, userId: userId1 });
  await svc.createRoom({ name: "Salon H2", type: "piece", width: 5, height: 4, householdId: h2, userId: userId2 });
...
test("createRoom : id en double rejeté", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  await svc.createRoom({ id: "room-fixe", name: "Salon", type: "piece", width: 5, height: 4, householdId , userId });
  const res = await svc.createRoom({ id: "room-fixe", name: "Autre", type: "piece", width: 3, height: 3, householdId , userId });
  assert.equal(res.success, false);
...
test("updateRoomPosition : cas nominal déplace bien la pièce", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const room = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId , userId });
  const res = await svc.updateRoomPosition(room.data.id, { x: 10, y: 3 }, userId);
  assert.equal(res.success, true);
...
test("updateRoomPosition : x/y non numériques rejetés", async () => {
  ...
  const { householdId, userId } = await makeHousehold();
  const room = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId , userId });
  const res = await svc.updateRoomPosition(room.data.id, { x: "beaucoup", y: 3 }, userId);
  assert.equal(res.success, false);
...
test("updateRoomPosition : déplacement qui chevaucherait une autre pièce est rejeté", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const salon = await svc.createRoom({ name: "Salon", type: "piece", width: 5, height: 4, householdId, x: 0, y: 0 , userId });
  const cuisine = await svc.createRoom({ name: "Cuisine", type: "piece", width: 3, height: 3, householdId, x: 10, y: 10 , userId });
...
test("updateRoomPosition : contact bord-à-bord exact (pas de chevauchement réel) est accepté", async () => {
  ...
  const res = await svc.updateRoomPosition(cuisine.data.id, { x: 5, y: 0 }, userId);
...
test("rectanglesOverlap : vérifie directement quelques cas géométriques", () => {
  ...
  assert.equal(svc.rectanglesOverlap(a, { x: 4, y: 0, width: 3, height: 3 }), true, "chevauchement partiel");
  assert.equal(svc.rectanglesOverlap(a, { x: 5, y: 0, width: 3, height: 3 }), false, "contact bord-à-bord exact");
  assert.equal(svc.rectanglesOverlap(a, { x: 100, y: 100, width: 3, height: 3 }), false, "aucun contact");
  assert.equal(svc.rectanglesOverlap(a, { x: 1, y: 1, width: 1, height: 1 }), true, "entièrement contenu");
});

/* -------------------------- type / icon (nouveau) -------------------------- */

test("createRoom : type manquant rejeté (désormais requis)", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const res = await svc.createRoom({ name: "Salon", width: 5, height: 4, householdId , userId });
  assert.equal(res.success, false);
  assert.match(res.error, /type/);
});

test("createRoom : type et icon acceptés et conservés tels quels", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const res = await svc.createRoom({ name: "Salon", type: "salon", icon: "🛋️", width: 5, height: 4, householdId , userId });
  assert.equal(res.success, true);
  assert.equal(res.data.type, "salon");
  assert.equal(res.data.icon, "🛋️");
});

test("createRoom : icon absente reste valide (optionnelle)", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const res = await svc.createRoom({ name: "Salon", type: "salon", width: 5, height: 4, householdId , userId });
  assert.equal(res.success, true);
  assert.equal(res.data.icon, null);
});

test("createRoom : dimensions non entières rejetées (cases de grille, pas des mètres flottants)", async () => {
  resetData();
  const { householdId, userId } = await makeHousehold();
  const res = await svc.createRoom({ name: "Salon", type: "salon", width: 5.5, height: 4, householdId , userId });
  assert.equal(res.success, false);
  assert.match(res.error, /width/);
});
